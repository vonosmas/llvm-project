  ...
  while (!valuesDefinedAbove.empty()) {
    for (mlir::Value val : valuesDefinedAbove) {
      ...
      if (mlir::isMemoryEffectFree(valOp)) {
        ...
        // Get a pointer to the first non-map argument (at the moment, the only
        // entry block argument-defining clause coming after `map` is `private`)
        // before modifying mapVars, then append an element to mapVars and an
        // associated entry block argument at that position.
        mlir::BlockArgument clonedValArg;
        auto privateBlockArgs = argIface.getPrivateBlockArgs();
        targetOp.getMapVarsMutable().append(mapOp);
        if (privateBlockArgs.empty())
          clonedValArg =
              region.addArgument(copyVal.getType(), copyVal.getLoc());
        else
          clonedValArg = region.insertArgument(
              privateBlockArgs.begin(), copyVal.getType(), copyVal.getLoc());
